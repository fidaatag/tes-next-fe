...
const PageEdit = ({params} : PageEditKelasProps) => {
  ...
  // get old data
  useEffect(() => {
    axios.get(`http://localhost:8000/api/admin/courses/${id}`, {
      headers: {"Authorization" : `Bearer ${token}`}
    })
    .then(response => {
      // console.log(response)
      set_DataOldForm(response.data.data)
    })
    .catch(error => {
      console.log(error)
    })
  ...
  useEffect(() => {
    if (isDataFormKelas_Filled()) {
      axios.put(`http://localhost:8000/api/admin/courses/${id}`, {
    ...
    <>
      <div className="p-3">
        ...
        <div className="w-full">
          ...
          <FormKelas typeBtn={typeButtonForm} AllValue={handleAllValueForm} oldData={data_OldForm}/>
